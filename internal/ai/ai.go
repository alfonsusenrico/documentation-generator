...
func (c *Client) GenerateProposal(template string, meta map[string]interface{}, plan string, enablePDF bool) (string, error) {
	...
	instructions := strings.Join([]string{
		...
		"Timeline hard rule: If plan states an MVP horizon (e.g., 3 weeks), keep dated phases within that horizon. For later phases, use 'Post-MVP (optional)' wording and avoid specific later week numbers unless explicitly provided.",
		"Timeline rule: Prefer relative schedule labels from plan. If implied but no exact dates, use realistic relative windows (Day 1-3, Week 1-3) while staying strictly within stated target horizon.",
